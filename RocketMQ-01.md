...
* Producer：消息的发送者；举例：发信者
  ...
  ```
  Producer先去NameServer中去查询Broker的信息，然后再根据信息和Broker建立连接，然后向Broker发送数据。
  ```

* Consumer：消息接收者；举例：收信者

  ```
  Consumer启动之后也是先去NameServer中查找信息，查看自己监听哪些队列，就和Broker建立连接了。
  由于RocketMQ支持多种消息拉取模式，例如：推模式、拉模式
  ```

* Broker：暂存和传输消息；举例：邮局

  ```
  Broker主要做消息处理的，比如数据储存，事务消息的回查等等
  ```

* NameServer：管理Broker；举例：各个邮局的管理机构

  ```
  NameServer中记录了Producer、Consumer、Broker、Topic信息,起控制中心的作用，也可以叫做路由作用。
  以前使用的zookeeper做控制中心，但是zookeeper的功能太多了，我们这里只需要一个控制中心的作用，所以阿里自己写了一个组件NameServer，代码也就1000多行
  ```
...
```
同步刷盘和异步刷盘的区别：异步刷盘就是broker接收到producer的消息后，直接返回生产者消息，最后再将数据写到磁盘。刷盘就是内存数据存到硬盘。什么时候存到硬盘就决定了是同步刷盘还是异步刷盘
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/store
#commitLog 存储路径
storePathCommitLog=/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/store/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/store/consumequeue
#消息索引存储路径
storePathIndex=/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/store/index
#checkpoint 文件存储路径
storeCheckpoint=/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/store/checkpoint
#abort 文件存储路径
abortFile=/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/store/abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
将xml文件中的${user.home}替换为/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release：

```
sed -i 's#${user.home}#/usr/local/rocketmq/rocketmq-all-4.7.0-bin-release#g' *.xml
```
...
```
# nameserver
122.51.83.231 rocketmq-nameserver1
192.168.25.138 rocketmq-nameserver2
# broker
122.51.83.231 rocketmq-master1
192.168.25.135 rocketmq-slave2
192.168.25.138 rocketmq-master2
192.168.25.138 rocketmq-slave1
...
122.51.83.231 rocketmq-nameserver1
# broker
122.51.83.231 rocketmq-


指定配置文件启动
nohup sh mqbroker -c /usr/local/rocketmq/rocketmq-all-4.7.0-bin-release/conf/2m-2s-async/broker-a.properties &
```



```
在集群里面可以看见结点，结点的名字是分片的名字，0代表主节点
要想生产TPS、消费TPS、今日生产消费，昨日生产消费准确，服务器上的时间必须准确
RocketMQ自身就带有一些主题，RocketMQ自身也带有重试机制
如果消息重试多次之后还是失败，那么就会被标记为死信，死信也可以重新发送的。
消费者：RocketMQ内置也有一些消费组
```



```
RocketMQ天然就支持集群，由四部分组成，Producer、NameServer、Broker、Consumer
```



```
RocketMQ可以发送普通消息、顺序消息、事务消息，顺序消息可以实现有序消费、事务消息可以解决分布式事务实现数据最终一致性。
```
...
```
RocketMQ是允许自动创建主题的，但是生产环境中是不允许自动创建主题的
Tag 标签 是用来过滤的，有时候需要查询对应的消息，主要用于消息过滤
keys 消息的唯一值，可以用它来定位消息
```



```
发送的消息会发送到topic下的默认的四个队列中，轮训发送到这四个
偏移量就是你在队列中读取数据的时候，第一次读到什么地方，第二次读到什么地方  就是一个记录
```
